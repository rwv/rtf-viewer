...
export function pixelSize(page: { width: number; height: number }, options: RenderOptions = {}) {
  const ppi = options.ppi ?? 96,
    scale = options.scale ?? 1,
    ratio = options.pixelRatio ?? 1;
  if (![ppi, scale, ratio].every((v) => Number.isFinite(v) && v > 0))
    throw new RangeError('PPI, scale and pixelRatio must be positive finite numbers.');
  ...
  if (!Number.isFinite(factor) || factor <= 0)
    throw new RangeError('Output scale exceeds the numeric range.');
  const width = ceilPixels(page.width * factor),
    height = ceilPixels(page.height * factor);
  if (
    ...
    throw new RangeError('Output canvas exceeds 32 million pixels or the dimension limit.');
    ...
    for (let lineIndex = 0; lineIndex < page.lines.length; lineIndex++) {
      ...
      for (const fragment of page.lines[lineIndex].fragments) {
        ...
        for (const position of [
          fragment.underline ? fragment.baseline + fragment.fontSize * 0.12 : null,
          fragment.strike ? fragment.baseline - fragment.fontSize * 0.3 : null,
        ]) {
          if (position === null) continue;
          context.beginPath();
          context.moveTo(fragment.x, position);
          context.lineTo(fragment.x + fragment.width, position);
          context.stroke();
        }
      }
    }
