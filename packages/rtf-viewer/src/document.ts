...
import type { DocumentLayout, LoadOptions, PageLayout, PageSize, RenderOptions, RtfInput } from './types.js';
...
export class RtfDocument {
  ...
  static async load(input: RtfInput, options: LoadOptions = {}): Promise<RtfDocument> {
    ...
    try {
      ...
      if (resources.fontEpoch !== epoch) throw new Error('Fonts changed during layout. Retry after fonts settle.');
      checkAbort(options.signal);
      return new RtfDocument(freezeDeep(model), resources, freezeDeep(layout));
    } catch (error) { resources.destroy(); throw error; }
  }
  get pageCount(): number { return this.layout.pages.length; }
  get layoutRevision(): number { return this.revision; }
  get needsRelayout(): boolean { return this.fontEpoch !== this.resources.fontEpoch; }
  get destroyed(): boolean { return this.closed; }
  get diagnostics(): readonly Diagnostic[] {
    return freezeDeep([...this.model.diagnostics, ...this.resources.diagnostics, ...this.layout.diagnostics]);
  }
  ...
  async relayout(options: { signal?: AbortSignal } = {}): Promise<void> {
    ...
    try {
      ...
      if (this.resources.fontEpoch !== epoch) throw new Error('Fonts changed during layout. Retry after fonts settle.');
      checkAbort(signal);
