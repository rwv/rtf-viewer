...
export async function parseInput(input: RtfInput, options: LoadOptions): Promise<DocumentModel> {
  ...
  if (!(input instanceof Blob) && !(input instanceof ArrayBuffer) && !(input instanceof Uint8Array))
    throw new TypeError('Expected a Blob, ArrayBuffer or Uint8Array.');
  ...
  if (!Number.isFinite(timeout) || timeout < 1 || timeout > 120_000)
    throw new RangeError('Invalid parser timeout.');
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(
      () => finish(new Error('RTF parsing exceeded its time limit.')),
      timeout,
    );
    worker.onmessage = ({ data }: MessageEvent<ParseResponse>) => {
      try {
        if (!data || typeof data !== 'object' || typeof data.ok !== 'boolean')
          throw new Error('Invalid parser Worker response.');
        if (!data.ok) {
        ...
        if (
          ...
          throw new Error('Unsupported RTF model schema.');
    ...
    worker.onerror = (event) => {
      ...
      finish(new Error(event.message || 'Parser Worker failed.'));
    };
    ...
    try {
      worker.postMessage({ bytes, wasmUrl }, [bytes]);
    } catch (error) {
      finish(error);
    }
  });
