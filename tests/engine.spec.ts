...
test('independent exact-line fixture paginates 15 + 9 with resolution-invariant geometry', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const bitmapSize = [bitmap.width, bitmap.height]; bitmap.close();
    const result = { pages: doc.pageCount, lines: [doc.getPageLayout(0).lines.length, doc.getPageLayout(1).lines.length], y: doc.getPageLayout(0).lines.map((l: any) => l.y), next: doc.getPageLayout(1).lines[0].fragments.map((f: any) => f.text).join(''), low, high, bitmapSize, closedWidth: bitmap.width, invariant: before === JSON.stringify([doc.getPageLayout(0), doc.getPageLayout(1)]) };
    doc.destroy(); return result;
  });
  expect(result).toMatchObject({ pages: 2, lines: [15, 9], low: [216, 216], high: [540, 540], bitmapSize: [432, 432], closedWidth: 0, invariant: true });
  expect(result.y[0]).toBe(18); expect(result.y[14]).toBe(186);
...
test('inline PNG and JPEG use authored dimensions and paint the declared regions', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const colors = images.map((f: any) => [...context.getImageData(Math.floor(f.x + 4), Math.floor(f.y + 4), 1, 1).data]);
    const result = { images: images.map((f: any) => [f.width, f.height]), colors, codes: doc.diagnostics.map((d: any) => d.code) };
    doc.destroy(); return result;
  });
  expect(result.images).toEqual([[36, 27], [36, 27]]);
...
test('abort terminates dedicated Workers and rejects pre-aborted loads', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    window.Worker = class extends NativeWorker {
      constructor(...args: ConstructorParameters<typeof Worker>) { super(...args); created++; }
    ...
    try {
      ...
      controller.abort();
      const during = await loading.then(() => 'resolved', (e: Error) => e.name);
      const before = await RtfDocument.load(input, { signal: controller.signal }).then(() => 'resolved', (e: Error) => e.name);
      return { created, terminated, during, before, inputBytes: input.byteLength };
    } finally { window.Worker = NativeWorker; }
  });
  expect(result).toMatchObject({ created: 1, terminated: 1, during: 'AbortError', before: 'AbortError' });
...
test('concurrent targets work; contention, destroy and borrowed ownership are deterministic', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const collision = await doc.renderPage(a, 0).then(() => 'resolved', (e: Error) => e.message);
    await Promise.all([first, doc.renderPage(b, 0)]);
    const borrowed = RtfViewer.fromDocument(a, doc);
    const borrowedLoad = await borrowed.load(new ArrayBuffer(0)).then(() => 'resolved', (e: Error) => e.message);
    borrowed.destroy();
    const survives = !doc.destroyed;
    const painting = doc.renderPage(a, 0); doc.destroy(); doc.destroy();
    const destroyedPending = await painting.then(() => 'resolved', (e: Error) => e.name);
    const after = await doc.renderPage(b, 0).then(() => 'resolved', (e: Error) => e.message);
...
test('rejects malformed input and image allocation bombs before browser decode', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const bomb = await RtfDocument.load(new TextEncoder().encode(image), { fonts: {} }).then(() => 'resolved', (e: Error) => e.message);
    return { bad, bomb };
  });
...
test('real LibreOffice sample matches independent page/text geometry and nearby ink', async ({ page }) => {
  ...
  const result = await page.evaluate(async (referenceBytes) => {
    ...
    const doc = await RtfDocument.load(await (await fetch('/samples/libreoffice-25.2.3.2.rtf')).arrayBuffer(), {
      fonts: { 'Liberation Serif': 'Rtf Liberation Serif', 'Liberation Sans': 'Rtf Liberation Sans' }, fallbackFont: 'Rtf Free Sans',
    });
    const canvas = document.createElement('canvas');
    await doc.renderPage(canvas, 0, { ppi: 96 });
    const actual = canvas.getContext('2d')!.getImageData(0, 0, canvas.width, canvas.height).data;
    const expectedBitmap = await createImageBitmap(new Blob([new Uint8Array(referenceBytes)], { type: 'image/png' }));
    const referenceCanvas = new OffscreenCanvas(expectedBitmap.width, expectedBitmap.height);
    const context = referenceCanvas.getContext('2d')!; context.drawImage(expectedBitmap, 0, 0); expectedBitmap.close();
    const expected = context.getImageData(0, 0, referenceCanvas.width, referenceCanvas.height).data;
    const w = canvas.width, h = canvas.height;
    const dark = (pixels: Uint8ClampedArray, x: number, y: number) => x >= 0 && y >= 0 && x < w && y < h && pixels[(y * w + x) * 4] < 150;
    let actualInk = 0, expectedInk = 0, unmatched = 0;
    const near = (pixels: Uint8ClampedArray, x: number, y: number) => {
      for (let dy = -4; dy <= 4; dy++) for (let dx = -4; dx <= 4; dx++) if (dark(pixels, x + dx, y + dy)) return true;
      return false;
    };
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
      if (dark(actual, x, y)) { actualInk++; if (!near(expected, x, y)) unmatched++; }
      if (dark(expected, x, y)) { expectedInk++; if (!near(actual, x, y)) unmatched++; }
    }
    const lines = doc.getPageLayout(0).lines;
    const result = { pages: doc.pageCount, size: doc.getPageSize(0), pixels: [w, h], lines: lines.map((line: any) => line.fragments.map((f: any) => f.text ?? '').join('')), positions: lines.map((l: any) => [l.x, l.y]), actualInk, expectedInk, unmatchedRatio: unmatched / (actualInk + expectedInk), png: canvas.toDataURL() };
    doc.destroy(); return result;
  }, referenceBytes);
  expect(result.pages).toBe(1); expect(result.size).toEqual({ width: 288, height: 360 });
  expect(result.pixels).toEqual([384, 480]);
  expect(result.lines).toEqual(['LibreOffice RTF', 'Reference', 'Plain text with bold, italic, and underlined', 'words.', 'Indented paragraph marker with', 'enough words to expose paragraph', 'geometry and line wrapping.', 'Unicode text: Hello, 中文, 你好世界.', 'Continuation Marker', 'This sentence follows the second heading.', 'Final marker: LO-25.2.3.2.']);
  ...
  await test.info().attach('engine-libreoffice-page', { body: Buffer.from(result.png.split(',')[1], 'base64'), contentType: 'image/png' });
});
...
test('malformed Worker replies terminate without waiting for the parser timeout', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    try {
      const error = await RtfDocument.load(new TextEncoder().encode('{\\rtf1 test}'), { workerUrl: url, parseTimeoutMs: 5000 }).then(() => 'resolved', (error: Error) => error.message);
...
test('an image bitmap that finishes decoding after load abort is closed', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    try {
      ...
      const error = await RtfDocument.load(bytes, { fonts: {}, signal: controller.signal }).then(() => 'resolved', (error: Error) => error.name);
...
test('destroying an owning viewer while loading aborts its pending acquisition', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const error = await loading.then(() => 'resolved', (error: Error) => error.name);
