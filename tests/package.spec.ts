...
test('the packed npm package works in an isolated production application', async ({ page }) => {
  ...
  const pkg = JSON.parse(await readFile('packages/rtf-viewer/package.json', 'utf8')) as { name: string; version: string };
  const tools = JSON.parse(await readFile('package.json', 'utf8')) as { devDependencies: Record<string, string> };
  ...
  try {
    await writeFile(join(temp, 'package.json'), JSON.stringify({ name: 'rtf-viewer-consumer', private: true, type: 'module' }));
    execFileSync('npm', ['install', '--ignore-scripts', '--no-audit', '--no-fund', archive,
      `vite@${tools.devDependencies.vite}`, `typescript@${tools.devDependencies.typescript}`], { cwd: temp, stdio: 'inherit' });
    const installedRoot = join(temp, 'node_modules', pkg.name);
    const installed = JSON.parse(await readFile(join(installedRoot, 'package.json'), 'utf8')) as { name: string; version: string };
    ...
    for (const file of ['dist/index.js', 'dist/index.d.ts', 'dist/viewer.js', 'dist/viewer.d.ts',
      'dist/parser.worker.js', 'dist/rtf_parser.js', 'dist/rtf_parser_bg.wasm', 'dist/generated/model.d.ts']) {
      expect(files).toContain(file);
    }
    for (const file of files) {
      expect(file).toMatch(/^(?:dist\/|package\.json$|README\.md$|LICENSE$|THIRD_PARTY_NOTICES\.md$|CHANGELOG\.md$)/);
    ...
    server = await preview({ root: temp, configFile: false, base: '/viewer/', preview: { host: '127.0.0.1', port: 0, strictPort: true } });
    ...
    const result = await page.evaluate(() => (window as unknown as { consumerResult: unknown }).consumerResult);
    expect(result).toEqual({ ok: true, pages: 2, lines: 15, bitmap: [432, 432], explicitAssets: true });
    ...
    const assets = await Promise.all((await readdir(join(temp, 'dist/assets'))).map(async file => ({
      file, bytes: (await stat(join(temp, 'dist/assets', file))).size,
    })));
    const manifest = { name: pkg.name, version: pkg.version, filename, sha256, integrity, bytes: bytes.length, files: files.sort() };
    const report = { package: pkg.name, version: pkg.version, filename, sha256, base: '/viewer/', result, failures, assets };
    await writeFile(join(artifacts, 'package-manifest.json'), JSON.stringify(manifest, null, 2) + '\n');
