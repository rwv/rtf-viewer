...
Initial hard defaults: 16 MiB input, 256 nesting depth, 2 million tokens, 2 million decoded text units, 100,000 blocks including page breaks, 256 embedded images, 8 MiB per image, 32 million decoded image pixels in total, 2,000 pages, and 32 million pixels per output canvas. Additional bounds are 16,384 graphemes per unbroken token, 2,048 pt font size/baseline magnitude, 14,400 pt paper/image dimensions, and 4 million pixels for a bitmap read out of a metafile, which is checked against the header before anything is allocated. That bitmap counts against the same 32-million decoded-pixel ceiling as any other picture. Each output axis is at most 32,767 pixels. Reject nonfinite/invalid geometry and output parameters. Diagnostics are deduplicated and bounded. These are engineering limits, not RTF specification maxima.

Font metrics are measured once per face at a large reference size and scaled, because a browser rounds font bounding box metrics to whole pixels and measuring at the document's size would make the line box depend on that size. The caller may declare the line box per font instead; the engine never infers a font's line gap, which Canvas does not expose and browsers disagree about through CSS.

Layout and paint yield periodically so AbortSignal and UI events can run. The cadence is counted once for the whole flow rather than at each call site, because a per-cell yield makes the cost of a table a function of its shape rather than of its size; the cadence is independent of the task primitive. `nextTask()` prefers `scheduler.yield()`, falls back to a `MessageChannel` task when that method is absent, and uses `setTimeout(0)` only if neither task API exists. Scheduler cancellation propagates to the caller. Each channel continuation owns both ports, removes its handler and closes both ports before resolving, including when its caller has been aborted or destroyed in the meantime; there is no persistent channel or shared callback queue. All paths resume in a later task, allowing browser input and rendering opportunities rather than only draining microtasks. The browser still chooses task ordering; there is no fixed latency guarantee.

Image decode promises are not intrinsically abortable; late decoded resources are always closed. Worker parsing timeout and byte bounds prevent a corrupt file from pinning the main thread. OLE and external-field content is never executed or fetched.
...
Verified with Vite production output in Chromium, Firefox and WebKit: the encoding and raster-size regressions, font lifecycle, and page/text geometry against one independently exported LibreOffice sample. The separate installed-tarball consumer verifies Worker/WASM rewriting in Chromium. Layout and paint latency are measured for four large shapes in Chromium and recorded in [verification](verification.md). Unverified assumptions: other bundlers; realistic broad CJK/complex-script coverage; Word/TextEdit output; vector metafile fidelity; layout latency at the configured upper limits and in engines other than Chromium. Common system font names alone cannot guarantee cross-platform pixel equality.
...
Task scheduling sources: [Prioritized Task Scheduling](https://wicg.github.io/scheduling-apis/#dom-scheduler-yield), [HTML timer nesting](https://html.spec.whatwg.org/multipage/timers.html#timers), and [MessagePort ownership](https://html.spec.whatwg.org/multipage/web-messaging.html#ports-and-garbage-collection).
