...
The committed LibreOffice 25.2.3.2 sample is compared to an independently exported PDF/PNG: page count, paper size, line text, indent coordinates and nearby-ink coverage are checked. With the line box those Liberation faces declare, 1.149902 em, passed through `lineHeights`, the engine's offset from the producer's nine measured markers is a constant -0.51 to -0.62 pt and does not accumulate, and the unmatched-ink ratio at 96 PPI is 0.0018. Without the declaration the browser's own metrics omit the font's line gap, the offset grows to -5.9 pt by the foot of the page, and the ratio is 0.083. Known metric/rasterization differences are documented in [compatibility](compatibility.md). Fixed test fonts keep this comparison reproducible.

The LibreOffice 24.2.7.2 table sample is compared to its own exported PDF, Poppler text extraction and rasterised pages. The engine produces the same three pages and the same line breaks inside cells: every line it emits appears unbroken in the producer's text extraction. Cell content left edges are offset by 0.40 to 0.45 pt of inset the producer adds and the file never asks for: its own drawn cell rules sit on the boundaries the engine computes, two columns declaring the same hairline left border differ from each other by 0.05 pt, and the document declares no `\li` or `\fi` anywhere, so that difference is recorded as accepted rather than pursued. Two differences remain open in `fixtures/corpus.json`. With the declared line box the row pitch is 23.999 pt against the producer's 24.15, down from 23.50, and the engine now breaks the Zone 01 row exactly where the producer does; the remaining 0.151 pt per row is not the line box, since the text document proves the declared box is the producer's, and by page three it has accumulated into a one-line rather than a one-row offset. `scripts/corpus.ts` keeps that record honest by failing when a hash, a file or a producer document stops matching the manifest.
