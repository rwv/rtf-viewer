...
Merged cells (`\clmgf`, `\clmrg`, `\clvmgf`, `\clvmrg`), nested tables (`\itap` above 1, `\nestrow`), repeating header rows (`\trhdr`) and keep-together rows (`\trkeep`) are separate features. Each keeps its fallback and reports a specific diagnostic; none of them is implied by ordinary table support.
