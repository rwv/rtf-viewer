...
The engine retains all 72,992 metafile bytes, resolves the authored 89.84 × 59.54 pt rectangle, and now draws the artwork there: it reads the 121 × 81 bitmap the producer embedded in the metafile and reports `approximated-metafile-bitmap`, because the blit's raster operations and clipping are not applied. Counting coloured pixels over the white page at 96 ppi, the engine draws 10,177 against the producer's 10,384, a 2.0 percent difference; the top colours of the decoded bitmap, `rgb(230, 238, 248)` and `rgb(32, 84, 147)`, are the source's `#e6eef8` and `#205493` exactly. The remaining difference is resampling: the producer rasterised at its own resolution and the engine scales that 121 × 81 source into the rectangle.

The metafile contains no vector drawing records at all. Its own records are two `META_STRETCHDIB` blits; the 31,932-byte EMF reassembled from its four `MFCOMMENT` escape records holds two `EMR_STRETCHDIBITS` records and nothing else. Both rtf.js renderers return an empty SVG for it. The measurement and what follows from it are in the [reuse evaluation](reuse-evaluation.md); the short version is that a DIB decoder, not a vector renderer, is what this class of document needs, which is what was built.

The two blits are a mask and an image: an 8 bpp `BI_RLE8` mask combined with `SRCPAINT` and a 24 bpp `BI_RGB` image combined with `SRCAND`. Over a white page the mask is a no-op, so the last decodable bitmap is the one the producer meant to show, and that is the one drawn. A metafile that carries real vector records still gets its placeholder and `unsupported-vector-image`, now with a message that says which of the two cases it is.
