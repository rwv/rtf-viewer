...
`LoadOptions.lineHeights` declares the line box, per em, for a font the document names, beside the `fonts` mapping that resolves it to a CSS family. A producer lays out with the font's ascent, descent and line gap; Canvas reports the first two, and no browser exposes the third — the one that exposes it through CSS `line-height: normal` disagrees with the others, so the engine does not guess. An application that knows which fonts it ships can declare the factor: every Liberation face is 1.149902, and declaring it makes the engine's line pitch match LibreOffice's exactly on the corpus documents. Omit it and the browser's own metrics are used, which is what earlier releases did. Only own properties of the object are read.
